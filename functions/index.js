...
// ── 一次性：寫入 v1.9.08 changelog（呼叫一次後刪除此 function）────────
exports.seedChangelogV1908 = onRequest(
  { region: REGION },
  async (req, res) => {
    if (req.query.key !== 'seed-v1908-once') {
      res.status(403).send('forbidden'); return;
    }
    const db = getFirestore();
    const col = db.collection('changelogs');

    // 把舊的 isLatest 清掉
    const prev = await col.where('isLatest', '==', true).get();
    const batch = db.batch();
    prev.forEach(d => batch.update(d.ref, { isLatest: false }));
    await batch.commit();
    ...
    await col.add({
      version:  'v1.9.08',
      date:     '2026-06-11',
      title:    '救護參考手冊改版、用量監控精準化、三本電子書',
      isLatest: true,
      items: [
        {
          subtitle: '📚 救護參考手冊全新改版',
          detail:   '「更多」頁的救護參考手冊改為 CSS 書本封面設計\n新增三本消防署電子書：救護技術員教科書（紅）、緊急醫療救護操作手冊（藍）、TECC 戰術緊急傷患救護訓練手冊（橄欖綠）\n點封面直接在 Safari 開啟電子書',
        },
        {
          subtitle: '🎨 頁面 Header 統一',
          detail:   '救護參考手冊頁面改為與「定訓與公告」相同的標題列樣式\n返回鍵右側顯示頁面標題，移除舊版綠色 banner 卡',
        },
        {
          subtitle: '📊 用量監控精準度提升',
          detail:   'Cloud Monitoring 改用固定 3600 秒時間桶取代動態對齊期間\n消除 Unix epoch 對齊偏差，今日讀取數與 Firebase Console 顯示更接近',
        },
        {
          subtitle: '⚙️ 部署機制修正',
          detail:   'Deploy Production 與 Deploy Preview 同時推送 gh-pages 導致的競爭衝突已修正\n兩個 workflow 加入序列化部署，避免 ref-lock 錯誤造成書本封面等更新無法上線',
        },
      ],
    });

    res.send('v1.9.08 seeded OK');
  }
);
